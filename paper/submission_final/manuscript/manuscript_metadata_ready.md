<!-- Metadata-ready copy of the P16 manuscript, generated by scripts/apply_submission_metadata.py --generate. It differs from the rendered P16 text only at the 16 external-metadata placeholders, which are named metadata tokens filled from paper/submission_final/metadata/METADATA_VALUES_TEMPLATE.yaml. Do not edit by hand. -->
...
**Corresponding author:** {{M03_CORRESPONDING_AUTHOR}}
...
delivered data and are not assumed here. Temperature and relative humidity were recorded with {{M06_SENSOR_MODEL}}, with a manufacturer-specified accuracy of {{M07_SENSOR_ACCURACY}}, a resolution of {{M08_SENSOR_RESOLUTION}} and a response time of {{M09_SENSOR_RESPONSE_TIME}}. Within the mat, the temperature–humidity sensor was located {{M10_SENSOR_PLACEMENT}}; relative to the heater, it was located {{M10_HEATER_RELATIVE_PLACEMENT}}.
...
night. The released data ({{M16_DATA_SCOPE}}) are available at {{M16_DATA_REPOSITORY_URL}} (persistent identifier: {{M16_DATA_IDENTIFIER}}) under the {{M16_DATA_LICENSE}} license.

The code for data preparation, model training, evaluation and the reproduction of the reported results is maintained
in a version-controlled repository; the data themselves are not part of it. The released code ({{M16_CODE_SCOPE}}) is available at {{M16_CODE_REPOSITORY_URL}} (archive identifier: {{M16_CODE_IDENTIFIER}}) under the {{M16_CODE_LICENSE}} license.
...
subject to final author and data-provider approval {{M16_HEATER_DIAGNOSTIC_APPROVAL}}. The public release therefore does not independently
