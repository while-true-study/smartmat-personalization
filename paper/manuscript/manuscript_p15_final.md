...
- **Concept drift:** the relation between inputs and target may also change over time [@gama2014survey]. For
  ...
  recursive updates and ensembles have been organized around this concept-drift view [@kadlec2011adaptation].
...
- **Compressed dynamics.** After fine-tuning, the variation of the predictions was strongly compressed: at b = 14 the
  prediction standard deviation was `{{p8_dynamic_summary | subject_id=User02, target=temperature, condition=E, budget_nights=14 | Q | .2f}}` times the target standard deviation for User02 temperature and
...
- **Scratch versus fine-tuning.** A network trained from scratch on the same adaptation nights was not worse than
  the pretrained model in {{COUNT:p8_interpretation_cases | case_I_S_approx_or_better_than_E=True}} of six subject–target cells at b = 14, and a constant computed from the adaptation labels
...
- **Why the same recipe helped one held-out domain and hurt another.** Full fine-tuning on a subject's earliest nights
  ...
  agrees with this account in {{COUNT:p6_level_mismatch_consistency | consistent=True}} of 24 cells (Section 4.5), but
  ...
  drift [@gama2014survey] and negative transfer from a less related source [@wang2019negative], here the user's own
  earliest nights, and the role of such offsets resembles field calibration of environmental sensors
  [@maag2018calibration; @delaine2019insitu]. These parallels are interpretive and were not tested; the observations do
...
- **A device-level residual.** User02's large zero-shot negative temperature bias was mostly corrected, yet mat 22482
  ...
  sensing [@stisen2015smart]; here, the mat cannot be separated from its recording conditions and heater history, and
...
- **Statistics:** the night-level bootstrap resamples nights independently; consecutive nights are correlated through
  ...
  uses the later span's labels; its agreement with {{COUNT:p6_level_mismatch_consistency | consistent=True}} of 24 cells is an association, not a causal test.
