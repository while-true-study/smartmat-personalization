...
We submit the manuscript **"Strict Unseen-Domain Evaluation of Smart-Mat Microclimate Estimation against Simple Level
Baselines"** for consideration as an Article in *Applied Sciences*,
...
- Post-hoc comparators, fixed before they were computed, show that these gains are mainly level corrections: a
  ...
  demonstrable within-subject tracking of temperature or humidity. A final diagnostic found no consistent
  within-night co-variation between the neural predictions and the targets. We report this negative result in full,
  scoped to the tested representation, architecture and training schedules.
- The conference paper's relative improvements among pressure representations are not disputed. The journal article
  shows that such relative improvements do not, by themselves, establish superiority over simple level baselines
  under strict subject-wise and chronological evaluation.
...
- **What has changed in the journal article:**
  ...
  - post-hoc comparators (personalized constant, offset-calibrated base model, scratch initialization control), a
    residual-variation analysis and a dynamic-signal (correlation) diagnostic;
