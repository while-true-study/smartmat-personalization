...
>   statement.
> - Nothing is cited from a search snippet alone. Kadlec 2011 was first retained on a search summary (pass 3). The
>   authors then confirmed it directly on the publisher page in the final integration pass, and it is now rated strong
>   within a restricted claim scope (see its row).
> - Candidate metadata from the authors' earlier manuscripts was re-checked and corrected: two author lists and one
>   venue were wrong there.
...
| kadlec2011adaptation | Kadlec, P.; Grbić, R.; Gabrys, B. Review of adaptation mechanisms for data-driven soft sensors. *Comput. Chem. Eng.* **2011**, 35, 1–24. doi:10.1016/j.compchemeng.2010.07.034 | review | YES | Adaptive data-driven soft sensing (indirect estimators) under changing process and data distributions; adaptation mechanisms (moving-window, recursive, ensemble-based) organised by concept-drift theory | **strong** (upgraded from moderate in the final integration pass): the authors confirmed the article directly on the publisher's page (Elsevier/ScienceDirect): *Comput. Chem. Eng.* 35(1), 1–24, 2011, doi:10.1016/j.compchemeng.2010.07.034, matching the Crossref record. Automated access still returns HTTP 403 | process industry. Supports **only** adaptive soft sensing, adaptation under distribution change and the adaptation frameworks. **Not** evidence for target-user personalization effectiveness, smart-mat personalization, a few-night adaptation benefit or negative transfer in this application | §2.4, §5.6 |
